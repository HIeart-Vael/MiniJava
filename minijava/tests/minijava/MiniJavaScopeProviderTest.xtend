...
class MiniJavaScopeProviderTest {
	...
	@Test def void testScopeProvider() {
		...
		'''.parse.classes.head.methods.last.returnStatement.expression => [
			assertScope(MiniJavaPackage.eINSTANCE.getMemberSelection_Member, "f, m, C.f, C.m")
			assertScope(MiniJavaPackage.eINSTANCE.getSymbolRef_Symbol, "v, p")
		]
	}
	...
	@Test def void testScopeProviderForSymbols() {
		'''
			...
			class A {}
		'''.parse.classes.head.methods.last.body.eAllContents.filter(VarDecl) => [
			findFirst[name == 'v3'].expression.assertScope(MiniJavaPackage.eINSTANCE.getSymbolRef_Symbol, "v2, v1, p")
			findFirst[name == 'v4'].expression.assertScope(MiniJavaPackage.eINSTANCE.getSymbolRef_Symbol, "v1, p")
		]
	}
	...
	@Test def void testVariableShadowsParamScoping() {
		...
		'''.parse.classes.head.methods.head => [
			returnStatement.expression.assertScope
				(MiniJavaPackage.eINSTANCE.getSymbolRef_Symbol, 
				"a")
		]
	}
	...
	@Test def void testVariableShadowsParamLinking() {
		...
		'''.parse.classes.head.methods.head => [
			...
			body.statements.head.assertSame(
				(returnStatement.expression as SymbolRef).symbol)
		]
	}
	...
	@Test def void testFieldsScoping() {
		...
		class D extends C {
			...
			MiniJavaPackage.eINSTANCE.getMemberSelection_Member, "b, m, a")
	...
	@Test def void testMethodsScoping() {
		...
		class D extends C {
			...
			MiniJavaPackage.eINSTANCE.getMemberSelection_Member, "m, f, n")
	...
	@Test def void testMemberScopeWithUnresolvedReceiver() {
		'''
			...
			class A {}
		'''.parse.classes.head.methods.last.returnStatement.expression => [
			assertScope(MiniJavaPackage.eINSTANCE.getMemberSelection_Member, "")
		]
	}
	...
	@Test def void testClassesInTheSamePackageInDifferentFiles() {
		...
		'''.parse(first.eResource.resourceSet)
		second.classes.head => [
			assertScope(MiniJavaPackage.eINSTANCE.getMemberDecl_Type, "D, B, C, apackage.D, apackage.B, apackage.C, anotherpackage.E")
		]
	}
	...
	@Test def void testLocalClassHasThePrecedenceOverTheSameClassInTheSamePackageInDifferentFiles() {
		...
		'''.parse(first.eResource.resourceSet)
		second.classes.head => [
			assertScope(MiniJavaPackage.eINSTANCE.getMemberDecl_Type, "C, B, apackage.C, apackage.B")
			assertSame(fields.head.type)
		]
	}

	def private returnExpSel(MethodDecl m) {
		m.returnStatement.expression as MemberSelection
	}
