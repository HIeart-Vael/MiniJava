 ...
 */
package org.xtext.projects.minijava.ui.contentassist
...
class MiniJavaProposalProvider extends AbstractMiniJavaProposalProvider {
	...
	override getStyledDisplayString(EObject element, String qualifiedNameAsString, String shortName) {
		if (element instanceof MemberDecl) {
			new StyledString(element.memberAsStringWithType).append(
				new StyledString(" - " + (element.eContainer as ClassDecl).name, StyledString.QUALIFIER_STYLER))
		} else
	...
	override void completeSelectionExpression_Member(EObject model, Assignment a, ContentAssistContext context,
		...
		lookupCrossReference(a.getTerminal() as CrossReference, context, acceptor) [ description |
			(description.getEObjectOrProxy as MemberDecl).isAccessibleFrom(model)
		]
	}
}
